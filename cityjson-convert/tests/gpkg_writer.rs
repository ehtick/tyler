...
fn converts_model_to_gpkg_with_feature_layers_relations_and_metadata() {
    ...
    assert_eq!(i32::from_le_bytes(blob[4..8].try_into().expect("srs id bytes")), 7415);
    assert_eq!(u32::from_le_bytes(blob[57..61].try_into().expect("wkb type bytes")), 1006);
...
fn convert_to_gpkg_requires_a_parseable_crs() {
    ...
    assert!(!output.exists(), "writer should fail before creating the file");
}
